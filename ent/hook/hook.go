...
// Mutate calls f(ctx, m).
func (f ObservacionFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ObservacionMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ObservacionMutation", m)
	}
	return f(ctx, mv)
}
...
// The TramiteFunc type is an adapter to allow the use of ordinary
// function as Tramite mutator.
type TramiteFunc func(context.Context, *ent.TramiteMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TramiteFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.TramiteMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TramiteMutation", m)
	}
	return f(ctx, mv)
}
